...
class MultiChannelReverb(torch.nn.Module):
    def __init__(self,sample_rate,ir_duration, init_ir=None, n_channels=6):
        super().__init__()
        self.sample_rate=sample_rate
        self.ir_duration=ir_duration
        ir_samples = int(ir_duration*sample_rate)
        if init_ir is None:
            init_ir = torch.rand((n_channels,ir_samples),dtype=torch.float32)*0.01
        else:
            assert init_ir.shape==torch.Size((n_channels,ir_samples))
    ...
    def get_ir(self):
        return torch.cat(
            (
            torch.zeros((self.n_channels,1),dtype=self.impulse_response_param.dtype,device=self.impulse_response_param.device)
             ,self.impulse_response_param[:,1:]
             ),
             -1)
    
    def forward(self,signal):
        impulse_response = self.get_ir()
        output = torch.nn.functional.conv1d(input=signal,weight=impulse_response[None,:,:], padding="same")
        return output
    
    def forward_without_channel_summation(self,signal):
        import matplotlib.pyplot as plt
        
        impulse_response = self.get_ir()
        output = torch.zeros(signal.shape, dtype=signal.dtype, device=signal.device)
        # print(impulse_response.shape)
        # # process each channel separately
        for i in range(self.n_channels):
            output[:,i,:] = torch.nn.functional.conv1d(input=signal[:,i:i+1,:],weight=impulse_response[i:i+1,None,:], padding="same")
        return output
...
class NaiveSynth(torch.nn.Module):
    '''
    Naive synthesizer that renders a sinusoidal model with harmonics
    Not trainable.
    '''
    def __init__(self, sample_rate) -> None:
        super().__init__()  
    ...
    def forward(self, features):
        assert "pitch" in features, "pitch is required"
        assert "rms" in features, "rms is required"
        assert "periodicity" in features, "periodicity is required"
        pitch = features["pitch"]
        rms = features["rms"]
        periodicity = features["periodicity"]
        assert pitch.shape == rms.shape, "pitch and rms must have the same shape but got {} and {}".format(pitch.shape, rms.shape)
        # check that dims is at least 2
        assert pitch.ndim >= 2, "pitch and rms must have at least 2 dimensions"
        # render with sinusoidal model with harmonics
        N_HARMONICS=30
        time = torch.arange(pitch.shape[-1], dtype=pitch.dtype, device=pitch.device)
        time = einops.repeat(time, "t -> b t", b=pitch.shape[0])
        # harmonics
        h = torch.arange(1, N_HARMONICS+1, dtype=pitch.dtype, device=pitch.device)
        # harmonic amps
        harmonic_amp = 1 / h
        # convert pitch to radians
        pitch = pitch[...,None] * 2 * np.pi * h[None, None,:]
        # compute phase
        phase = torch.cumsum(pitch, dim=1) / self.sample_rate
        # compute waveform
        waveform = torch.sin(phase)
        # apply harmonic amps
        waveform = waveform * harmonic_amp[None, None, :]
        # make white noise
        # apply periodicity
        waveform=waveform.sum(dim=-1)
        noise = torch.randn_like(waveform)
        periodicity = 1-((1-periodicity)**2)
        waveform = waveform * periodicity + noise * (1-periodicity) *0.1
